defmodule HeroDigitalWeb.PlateRegistrationDataController do
  ...
  def create(conn, %{"lead_id" => lead_id ,"plate_registration_data" => plate_registration_data_params}) do
    with {:ok, %PlateRegistrationData{} = plate_registration_data} <- PlateRegistration.create_plate_registration_data(Map.put(plate_registration_data_params, "lead_id", lead_id)) do
      conn
      |> put_status(:created)
